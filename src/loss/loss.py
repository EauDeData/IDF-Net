...
def minimum_transform_loss(features: torch.tensor, gt_distances: torch.tensor, similarity: Callable = CosineSimilarityMatrix, eye: bool = False) -> torch.tensor:
    '''
    ...
    if eye:
        mask_diagonal = ~ torch.eye(h_distances.shape[0]).bool()
        h_distances_eyed = h_distances[mask_diagonal].view(h_distances.shape[0], h_distances.shape[0] - 1) # Shape: (Bs x Bs-1)
        gt_distances_eyed = gt_distances[mask_diagonal].view(h_distances.shape[0], h_distances.shape[0] - 1) # Shape: (Bs x Bs-1)
    else:
        # I'm not sure about the consequences of this
        h_distances_eyed = h_distances
        gt_distances_eyed = gt_distances


    # Calculate transformation matrix
    T = torch.matmul(torch.linalg.pinv(h_distances_eyed), gt_distances_eyed)
